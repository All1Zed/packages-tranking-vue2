<template>
  <div>
    <NavView bcolor="green" pagetitle="Collection" />
    <v-container >
      ...
      <v-card elevation="1" max-width="769" class="mx-auto">
        <v-card-text class="text-center">
          ...
          @keyup.enter="collection(tracking_number)"
          @click:append-outer="collection(tracking_number)"
          ></v-text-field>
          <v-btn class="my-2" color="green" dark  @click="collection(tracking_number)">Check</v-btn>
    ...
    <v-dialog max-width="768" class="mx-auto" v-model="scannerD" persistent>
      <v-card color="blue">
        ...
        <ScannerView v-if="scannerD" class="pa-1" @scanned="getPackage($event)" />
      </v-card>
    </v-dialog>
      ...
      <v-card v-if="packagePreview">
        ...
        <v-card-actions>
          <div class="text-right">
            ...
            <v-btn color="primary" class="mx-2" @click="collect">Collected</v-btn>
          </div>
        </v-card-actions>
      </v-card>
    </v-dialog>
    ...
    <v-dialog max-width="768" persistent v-model="intransit_dialog">
      <v-card>
        <v-app-bar dark dense flat color="primary">Select New Vehcle</v-app-bar>
        <v-card-text>
    ...
    methods:{
      collection(){
        if(this.tracking_number){
          ...
          this.axios.patch('https://packages.pridezm.com/api/packages/'+this.tracking_number, {collected_status: true, transit_status: false }, {headers}).then((res) =>{
            if (res.status == 200) {
            ...
            this.feed_title = 'ERROR'
            this.feed_message = err.response.data.detail
            this.err = true
            this.feed_dialog =true
            this.loading = false
          })
