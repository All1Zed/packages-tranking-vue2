<template>
  <div>
    <NavView bcolor="orange" pagetitle="TRANSIT" />
    <v-container >
      ...
      <v-card elevation="1" max-width="769" class="mx-auto">
        <v-card-text class="text-center">
          ...
          @keyup="checkPackage(tracking_number)"
          @click:append-outer="checkPackage(tracking_number)"
          ></v-text-field>
          <v-btn class="my-2" dark color="green"  @click="checkPackage(tracking_number)">check</v-btn>
          <h1 class="my-1 font-weight-light">OR</h1>
          <v-btn @click="scannerD = !scannerD" dark color="orange" class="white--text lighten-2">scan qrcode</v-btn>
        </v-card-text>
      </v-card>
    </v-container>
    ...
    <v-dialog max-width="768" class="mx-auto" v-model="scannerD" persistent>
      <v-card color="blue">
        ...
        <ScannerView v-if="scannerD" class="pa-1" @scanned="checkPackage($event)" />
      </v-card>
    </v-dialog>
